...
namespace conColors {
	static inline CONSOLE_SCREEN_BUFFER_INFOEX getScreenBuffer() noexcept
	{
	...
	static inline void setScreenBuffer(CONSOLE_SCREEN_BUFFER_INFOEX scrBuffer, int attributeInd) noexcept
	{
		HANDLE conHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	static inline void setBackground(int r, int g, int b, int attribute = 0) noexcept
	{
	...
	static inline void setForeground(int r, int g, int b, int attribute = 0) noexcept
	{
		auto scrBuffer = getScreenBuffer();
